...
void test_init_worlds(const char* filepath,const char* dir){
    const Engine* engine = engine_get();
...
void test_get_levels(char** names, int* amount){
...
void test_load_level(char* name){
    const Engine* eng = engine_get();
    (void)eng;
...
void create_levelLoader_provider(void* engine){
    loader.initWorlds = test_init_worlds;
    loader.getLevels = test_get_levels;
    loader.loadLevel = test_load_level;
